...
[allAddr,allTs]=loadaerdat(curSeqName,200000);
...
binomN = 4;
% binomial kernal 121 corresponds to sigma=1/2 sqrt(n) with n=3
% and effective sigma of two consecutive Gaussians has relation sigma^2 =sigma_1^2+sigma_2^2
% and Gaussian used here has actual sigma of sigma/(2*pi)
sigmaLR = sqrt((sigma/(2*pi))^2 - binomN/4)/2*2*pi;
% sigmaLR = sqrt((sigmaLR/(2*pi))^2 - binomN/4*2)/2*2*pi;  % double of corresponding sigma because of subsampling

f0LR = f0*2;
[~, gabor0LR] = getGaborLookupAndFilt(0, sigmaLR, f0LR, plotTrue);
[~, gabor90LR] = getGaborLookupAndFilt(90, sigmaLR, f0LR, plotTrue);
...
dt = 15E3; % in mus % sample rate of temporal response function
...
fprintf('\nWARNING: Adjust if parameters change\n\n');
bufferTs = [0, dt, 2*dt];
valsForBufferTmonoTemplate = [0, .8, .4];
valsForBufferTbiTemplate = [0, -.4, .8];
...
plot(bufferTs/1E3, valsForBufferTmonoTemplate, '-og', ...
    bufferTs/1E3, valsForBufferTbiTemplate, '--ob', ...
    'LineWidth', 2);
xlabel('t [ms]')
title(sprintf('Temporal filters (muBi1=%6.3f)', muBi1));

testStim = zeros(21, 21); testStim(11, 11) = 1;


testLR = imfilter(imfilter(testStim, [1 4 6 4 1]/16), [1 4 6 4 1]'/16);
testLR = testLR(1:2:end, 1:2:end);
testLR = imfilter(testLR, imag(gabor90LR));

subplot 224;
imshow(testLR, []); colorbar


subplot 223;
% imshow(imfilter(testStim, imag(gabor90)), []); colorbar
imshow(imresize(imfilter(testStim, imag(gabor90)), size(testLR)), []); colorbar
drawnow;
%%
figure(11);
...
for tInd = 1:numel(ts)
    ...
    if curT > nextPlotTime
        ...
        filtResp0Approx = getFiltRespFromTempBuffLowRes( ...
            curTempFilteredBuffRespBiApprox, curTempFilteredBuffRespMonoApprox, gabor0LR, borderCond);
        filtResp90Approx = getFiltRespFromTempBuffLowRes( ...
            curTempFilteredBuffRespBiApprox, curTempFilteredBuffRespMonoApprox, gabor90LR, borderCond);
...
%         fprintf('%8.5f, %8.5f\n', min(filtResp0Approx(:)), max(filtResp0Approx(:)));
        ...
        subSamp = 4; quiverScale = 10*subSamp;
        ...
        % approx
        subSampLR = 2; quiverScaleLR = 10*subSampLR;
        
        sigmaPlotBlurrLR = sigmaPlotBlurr*subSampLR/subSamp;
        HmotBlurrLR = fspecial('Gaussian', 4*sigmaPlotBlurrLR+1, sigmaPlotBlurrLR);
        
        respPlot90Approx = imfilter(filtResp90Approx, HmotBlurrLR);
        respPlot0Approx = imfilter(filtResp0Approx, HmotBlurrLR);
        colorWheelCodedPlotApprox = getColorWheelCoded(respPlot0Approx, respPlot90Approx, maxPlotSpeed);
        
        subplot(3,3,2);
        maxYLR = size(respPlot90Approx, 1);
        maxXLR = size(respPlot90Approx, 2);
        imshow(colorWheelCodedPlotApprox); hold on;
        quiver(1:subSampLR:maxXLR, 1:subSampLR:maxYLR, ...
            respPlot90Approx(1:subSampLR:maxYLR,1:subSampLR:maxXLR)*quiverScaleLR, ...
            respPlot0Approx(1:subSampLR:maxYLR,1:subSampLR:maxXLR)*quiverScaleLR, ...
            'Autoscale', 'off');
        axis ij equal; xlim([1, maxXLR]); ylim([1, maxYLR]);
        title(sprintf('Mot vecs (sigma=%4.1f)', sigmaPlotBlurrLR)); hold off;
        
        
        subplot(3, 3, 5);
        imshow(respPlot0Approx, respPRange);
        title('Vert. motion response (approx)');
        
        subplot(3, 3, 8);
        imshow(respPlot90Approx, respPRange);
        title('Hor. motion response (approx)');
